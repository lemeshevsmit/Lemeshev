package com.shpp.p2p.cs.olemeshev.assignment6.hg;
...
public class HistogramEqualization extends WindowProgram implements ComponentListener {
    ...
    private void computeEqualizedColorImage() {
        assert this.displayImage != null;
        int[][] picture = (this.displayImage).getPixelArray();
        int[][] red = new int[picture.length][picture[0].length];
        int[][] green = new int[picture.length][picture[0].length];
        int[][] blue = new int[picture.length][picture[0].length];
        for (int i = 0; i < picture.length; i++) {
            for (int j = 0; j < picture[0].length; j++) {
                red[i][j] = GImage.getRed(picture[i][j]);
                green[i][j] = GImage.getGreen(picture[i][j]);
                blue[i][j] = GImage.getBlue(picture[i][j]);
            }
        }
        int[][] intensities = HistogramEqualizationImageTransforms.imageToLuminances(picture);
        int[][] result = HistogramEqualizationLogic.equalize(intensities);
        int[][] eRed = HistogramEqualizationLogic.equalize(red);
        int[][] eGreen = HistogramEqualizationLogic.equalize(green);
        int[][] eBlue = HistogramEqualizationLogic.equalize(blue);
        if (this.confirmResult(result, picture)) {
            for (int row = 0; row < result.length; ++row) {
                for (int col = 0; col < result[row].length; ++col) {
                    result[row][col] = GImage.createRGBPixel(eRed[row][col], eGreen[row][col], eBlue[row][col]);
                }
            }
            this.setImage(new GImage(result));
        }
    ...
    private void equalizeImage() {
        if (this.displayImage == null) {
            ...
            //////////////////////////////////////color///////////////////////////////////////////
            //this.computeEqualizedColorImage();
