...
/**
 * This class display text name of color from input RGB value
 *
 * @author Aleksandr Lemeshev
 * @since 05.08.2022
 */
public class ColorTextName {
    ...
    /**
     * This method get the color name from our database of color (list)
     *
     * @param inputColor input color to analyze
     * @return text name of color from input RGB value
     */
    public static String getColorName(Color inputColor) {
        if (inputColor.getAlpha() == 0 & inputColor.getBlue() == 0 &
                inputColor.getGreen() == 0 & inputColor.getRed() == 0) {
            return "Transparent";
        }
        ArrayList<ColorName> colorList = initColorList();
        ColorName textColorName = null;
        int min = Integer.MAX_VALUE;
        int actualColor;
        for (ColorName color : colorList) {
            actualColor = color.calculateColor(inputColor);
            if (actualColor < min) {
                min = actualColor;
                textColorName = color;
            }
        ...
        if (textColorName != null) {
            return textColorName.getName();
        } else {
            return "Unknown color text name.";
        }
    }

    /**
     * Inner class  for save color parameter
     *
     * @author Aleksandr Lemeshev
     * @since 05.08.2022
     */
    public static class ColorName {
        ...
        /**
         * this method return int value of color
         *
         * @param color input color to analyze
         * @return int value of color
         */
        public int calculateColor(Color color) {
            int pixR = color.getRed();
            int pixG = color.getGreen();
            int pixB = color.getBlue();
